...
import { Search, Plus, Edit2, Trash2, Users, Star, Clock, MapPin, ChevronUp, ChevronDown, X } from "lucide-react";
...
const statusStyle = {
  "On Duty":  { bg: "#edf3ee", color: "#3a5a40", dot: "#3a5a40" }, // olive
  "Break":    { bg: "#faf6f0", color: "#8b6f4c", dot: "#8b6f4c" }, // amber
  "Off Duty": { bg: "#fafafa", color: "#cccccc", dot: "#cccccc" }, // netral
};
    ...
    <div className="p-6 space-y-5" style={{ background: "#f5f5f0", minHeight: "100%" }}>
      ...
      <div className="grid grid-cols-4 gap-4 fade-up-1">
        {[
          ...
          { label: "Avg Performance", value: `${Math.round(staffList.reduce((sum, s) => sum + s.performance, 0) / staffList.length)}%` },
          { label: "Top Performer",   value: staffList.reduce((top, s) => s.performance > top.performance ? s : top, staffList[0]).name.split(" ")[0] },
        ].map(card => (
          <div key={card.label} className="rounded-xl p-4" style={{ background: "#ffffff", border: "1px solid #e8e8e3" }}>
            <p className="text-xs uppercase tracking-widest mb-2" style={{ color: "#cccccc", letterSpacing: "0.1em" }}>{card.label}</p>
            <p className="text-xl font-semibold" style={{ color: "#111111", fontFamily: "'Playfair Display', serif" }}>{card.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 fade-up-2">
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg flex-1 max-w-[220px]" style={{ background: "#ffffff", border: "1px solid #e8e8e3" }}>
        ...
        <FilterGroup label="Role" options={roles} selected={roleFilter} setSelected={setRoleFilter} />
        <FilterGroup label="Status" options={statuses} selected={statusFilter} setSelected={setStatusFilter} />
        <FilterGroup label="Shift" options={shifts} selected={shiftFilter} setSelected={setShiftFilter} />

        <button
          onClick={() => setSortDir(d => d === "desc" ? "asc" : "desc")}
          className="flex items-center gap-1 text-xs px-3 py-2 rounded-lg ml-auto"
          style={{ background: "#ffffff", border: "1px solid #e8e8e3", color: "#555555" }}
        >
          Performance {sortDir === "desc" ? <ChevronDown size={12} /> : <ChevronUp size={12} />}
        </button>
      </div>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <div className="flex items-center justify-center rounded-full flex-shrink-0" style={{ width: 28, height: 28, background: "#f5f5f0", color: "#aaaaaa", fontSize: 11, fontWeight: 600 }}>
                  {staff.name.charAt(0)}
                </div>
                <div>
                  <p className="font-medium" style={{ color: "#111111" }}>{staff.name}</p>
                  <p className="text-xs" style={{ color: "#cccccc" }}>{staff.role}</p>
                </div>
              </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.3)" }}>
          <div className="rounded-xl p-6 w-full max-w-md" style={{ background: "#ffffff", border: "1px solid #e8e8e3" }}>
